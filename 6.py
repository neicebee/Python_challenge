...
A = get_html('http://www.pythonchallenge.com/pc/def/channel.html')
p1 = re.compile('<!--(.+?)-->')
p2 = re.compile('[a-z]+')
A.change_url(A.check_url().replace(
    'html',
    p2.findall(p1.findall(A.get_req().replace('\n', ''))[0])[0]
))
...
# import os
# os.system("unzip channel.zip -d channel/")
